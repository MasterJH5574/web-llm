...
def decode_matmul_pattern(match_ewise: bool, dtype: str):
    w_scaled = wildcard()
    scale_min = wildcard()
    scale = wildcard()
    min_value = wildcard()
    ...
    if dtype != "float16":
        ...
        w = is_op("relax.call_tir")(
            GlobalVarPattern(), TuplePattern([w_scaled, scale, min_value]), add_constraint=False
        )
    matmul_args = [x, w]
    if match_ewise:
...
class FuseDecodeMatmulEwise:
    ...
    def transform_module(self, mod: IRModule, ctx: tvm.transform.PassContext) -> IRModule:
        mod = relax.transform.FuseOpsByPattern(
            [("decode_matmul", *decode_matmul_pattern(match_ewise=False, dtype=self.dtype))]
        )(mod)
        mod = relax.transform.FuseOpsByPattern(
            [("decode_matmul_ewise", *decode_matmul_pattern(match_ewise=True, dtype=self.dtype))]
        )(mod)
